...
3. Apply [the full checklist](references/checks.md), every check, including the ones `debloat-file` has to skip: duplication across files, circular imports, and whether a rules-file glob is actually selective in practice given the project's real file layout. Duplication is the check most likely to be under-run on a single read: follow the checklist's "Redundant or duplicated" section literally, including its topic-by-topic second pass and its instruction to check any file that cites another as authoritative against what that file actually says, before concluding the project has none.
